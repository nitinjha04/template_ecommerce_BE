...
class PaymentService {
    static async getAllAdmin(query) {
        ...
        if (regex) {
            const matchingOrders = await models_1.Order.find({ orderNumber: regex })
                .select('_id')
                .lean();
    ...
    static async getMyPayments(userId) {
        const payments = await models_1.Payment.find({ user: userId })
            .sort({ createdAt: -1 })
    ...
    static async updateStatus(id, status) {
        const payment = await models_1.Payment.findByIdAndUpdate(id, { status }, { new: true, runValidators: true }).populate('order', 'orderNumber total status');
        if (!payment) {
            throw new ApiError_1.ApiError(404, 'Payment not found');
        }
        return (0, serializePayment_1.serializePayment)(payment);
