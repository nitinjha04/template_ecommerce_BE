...
const buildCollectionUrl = (handle) => `https://empress-clothing.com/collections/${handle}`;
const validateEntry = (entry, index) => {
    if (!entry || typeof entry !== 'object') {
        throw new Error(`categories[${index}]: must be an object`);
    }
    const row = entry;
    const collectionHandle = String(row.collectionHandle ?? row.categoryFilter ?? '').trim();
    ...
    const config = {
        collectionHandle,
        collectionUrl: String(row.collectionUrl ?? '').trim() || buildCollectionUrl(collectionHandle),
        categoryName,
        priceMin,
        priceMax,
