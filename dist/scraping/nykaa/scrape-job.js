...
const map_nykaa_product_1 = require("./map-nykaa-product");
const price_quota_1 = require("./price-quota");
...
const loadRawPool = (filePath) => {
    ...
    if (parsed &&
        ...
        'response' in parsed &&
        parsed.response?.products) {
        return parsed.response.products;
    }
...
const runScrapeJob = async (config, options = {}) => {
    ...
    try {
        console.log('\n' + '─'.repeat(50));
        console.log(`▶ ${config.categoryName} (filter ${config.categoryFilter}) | ₹${config.priceMin}–${config.priceMax} | limit ${needed} | sort ${sort}`);
        let pool;
        let pagesFetched = 0;
        if (options.fromRaw) {
        ...
        for (let i = 0; i < needed; i += 1) {
            ...
            const doc = (0, map_nykaa_product_1.mapNykaaToProductDocument)(raw, {
                categoryName: config.categoryName,
                categoryFilter: config.categoryFilter,
                assignedPrice,
                index: i,
