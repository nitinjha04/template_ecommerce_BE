...
const parseArgs = (): CliArgs => {
  ...
  const only: CliArgs['only'] =
    onlyRaw === 'remove' || onlyRaw === 'prices' ? onlyRaw : 'all';
...
const maxOriginalForPrice = (price: number): number => {
...
const hasBlockedImage = (images: string[] | undefined): boolean => {
...
const resolveStoreFilter = async (
  ...
  if (args.storeId) {
    ...
    return { store: new Types.ObjectId(args.storeId) };
  }
...
const main = async (): Promise<void> => {
  ...
  if (args.only === 'all' || args.only === 'remove') {
    const urlRegex = BLOCKED_URL_PATTERNS.map((p) =>
      p.replace(/\./g, '\\.')
    ).join('|');
    ...
    for (const p of confirmed.slice(0, 20)) {
      const sample = (p.images ?? []).find((u) =>
        BLOCKED_URL_PATTERNS.some((pat) => u.toLowerCase().includes(pat))
      );
      console.log(`  [-] ${p.name?.slice(0, 55)}… (${p.slug})`);
      if (sample) console.log(`      img: ${sample.slice(0, 90)}`);
    }
  ...
  if (args.only === 'all' || args.only === 'prices') {
    ...
    const updates: { _id: Types.ObjectId; oldOriginal: number; newOriginal: number; price: number; name: string }[] =
      [];
    ...
    for (const p of products) {
      ...
      if (Math.round(original) === nextOriginal) continue;
    ...
    for (const row of updates.slice(0, 15)) {
      ...
      console.log(
        `  [~] ${row.name.slice(0, 45)}… ₹${row.price}  original ${row.oldOriginal} (${oldPct}% off) → ${row.newOriginal} (${newPct}% off)`
      );
    }
      ...
      for (let i = 0; i < updates.length; i += BATCH) {
        ...
        await Promise.all(
          chunk.map((u) =>
            Product.updateOne(
              { _id: u._id },
              { $set: { originalPrice: u.newOriginal } }
            )
          )
        );
      }
  ...
  if (examined) console.log(`Products examined (prices): ${examined}`);
