...
const parseArgs = () => {
  ...
  const domain =
    domainIdx >= 0 && argv[domainIdx + 1] ? argv[domainIdx + 1].trim() : '';
...
const main = async () => {
  ...
  const stores = await Store.find({ domain: { $in: domains } })
  ...
  for (const store of stores) {
    ...
    if (dryRun) {
      const sample = await Product.find(filter)
        .select('name category breadcrumbCategory')
        .limit(3)
        .lean();
      for (const row of sample) {
        console.log(
          `  would set "${row.breadcrumbCategory}" → "${row.category}" (${row.name?.slice(0, 40)})`
        );
      }
      continue;
    }
