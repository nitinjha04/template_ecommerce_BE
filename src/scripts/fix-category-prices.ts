...
type Tier = {
  /** Match against product.category (case-insensitive, partial) */
  match: RegExp;
  /** Sale price must be ≥ this */
  minPrice: number;
  /** Upper bound for remapping underpriced items */
  maxPrice: number;
};
...
const DEFAULT_TIER: Tier = { match: /.*/, minPrice: 400, maxPrice: 1999 };
...
const resolveTier = (category: string): Tier => {
  const cat = (category ?? '').trim();
  for (const tier of CATEGORY_TIERS) {
    if (tier.match.test(cat)) return tier;
  }
  return DEFAULT_TIER;
};
...
const originalForSalePrice = (salePrice: number): number => {
...
const resolveStoreFilter = async (args: {
  storeId?: string;
  storeDomain?: string;
}): Promise<{ filter: Record<string, unknown>; label: string }> => {
  if (args.storeId) {
    if (!Types.ObjectId.isValid(args.storeId)) {
      throw new Error(`Invalid --store-id: ${args.storeId}`);
    }
    return { filter: { store: new Types.ObjectId(args.storeId) }, label: args.storeId };
  }

  const domainOrSlug =
    args.storeDomain?.trim() ||
  ...
  if (!store) {
    throw new Error(
      `Store not found for "${domainOrSlug}". Pass --store-id or --store-domain explicitly.`
    );
  }
...
const main = async (): Promise<void> => {
  ...
  const byCategory = new Map<string, { fixed: number; kept: number; floor: number }>();
  ...
  console.log('\nPer category:');
  for (const [cat, s] of [...byCategory.entries()].sort((a, b) =>
    a[0].localeCompare(b[0])
  )) {
    console.log(
      `  ${cat || '(empty)'}: floor ₹${s.floor} | fix ${s.fixed} | ok ${s.kept}`
    );
  }
  ...
  for (const u of updates.slice(0, 20)) {
    console.log(
      `  [~] ${u.category} | ${u.name.slice(0, 40)}… ₹${u.oldPrice}→${u.newPrice} (was orig ${u.oldOriginal}→${u.newOriginal}) floor ${u.minFloor}`
    );
  }
  ...
  for (let i = 0; i < updates.length; i += BATCH) {
    ...
    await Promise.all(
      chunk.map((u) =>
        Product.updateOne(
          { _id: u._id },
          { $set: { price: u.newPrice, originalPrice: u.newOriginal } }
        )
      )
    );
  }
  ...
  const stillLow = await Product.countDocuments({
    ...filter,
