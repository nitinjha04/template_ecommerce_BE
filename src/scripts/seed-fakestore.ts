/**
 * Fetches products from Fake Store API and seeds MongoDB with unique variants.
 *
 * Usage:
 *   npm run seed:fakestore
 *   npm run seed:fakestore:dry
 */
...
const mapStoreToShopCategory = (storeCategory: string): ProductCategory => {
  const key = storeCategory.toLowerCase().trim();
  if (key === "men's clothing") return 'Men';
  if (key === "women's clothing") return 'Women';
...
const fetchAllFakeStoreProducts = async (): Promise<FakeStoreProduct[]> => {
  return fetchJson<FakeStoreProduct[]>(FAKESTORE_URL);
};

const trimDescription = (text: string, max = 500): string => {
  const t = text.trim();
  if (t.length <= max) return t;
  return `${t.slice(0, max - 3)}...`;
};

const buildDraft = (item: FakeStoreProduct): ProductDraft => {
  const baseName = item.title.trim();
  const storeCategory = item.category.toLowerCase().trim();
  const sourceKey = `fs-${item.id}`;
  const slugBase = `${slugify(baseName)}-fs${item.id}`;
...
const fillDrafts = (
  ...
  if (drafts.length < TARGET_PRODUCTS) {
    console.warn(
      `Only generated ${drafts.length}/${TARGET_PRODUCTS} (pool size: ${items.length})`
    );
  }
...
const main = async (): Promise<void> => {
  ...
  const globalNames = new Set(
    dryRun
      ? []
      : (await Product.find({}, { name: 1 }).lean()).map((p) => p.name)
  );
  ...
  for (const draft of drafts) {
    ...
    const slug = await uniqueSlug(draft.slug, Product as Model<unknown>);
