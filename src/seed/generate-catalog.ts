...
import { catalogImages, validateImageUrl } from './catalog-images';
...
const desc = (name: string, material: string): string =>
  `${name} crafted from ${material}. Easy to style and built to last. Designed for everyday wear with a refined, modern silhouette.`;

type Row = { name: string; tags: string[]; featured?: boolean };
...
const build = (
  ...
  rows.map((row, i) => ({
    ...
    images: catalogImages(row.name, category),
    tags: row.tags,
    inStock: true,
...
const catalog: CatalogProduct[] = [
  ...
  ...build(
    ...
    ONE_SIZE,
    accColors,
    'quality materials and thoughtful craftsmanship'
  ),
];
...
const main = async (): Promise<void> => {
  ...
  for (const product of catalog) {
    for (const url of product.images) {
      ...
      if (!ok) failures.push(`${product.name}: ${url}`);
    }
  }
