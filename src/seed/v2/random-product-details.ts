...
import type { V2ProductRaw } from './map-v2-product';
...
const resolveOriginalPrice = (raw: V2ProductRaw, salePrice: number): number => {
  const base = raw.basePrice ?? 0;
  if (base > salePrice) return Math.round(base);
...
export const buildRandomProductDetails = (
  raw: V2ProductRaw,
