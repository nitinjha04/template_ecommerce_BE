 ...
 */
export const ALL_PRICE_TIERS = [
  100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 2000, 3000, 4000, 5000, 6000,
  7000,
] as const;

const LOW_TIERS_EXCEPT_400 = [100, 200, 300, 500] as const;
const LOW_POOL_TOTAL = 200;
const TIER_400_COUNT = 15;
const HIGH_TIER_COUNT = 15;

export type PriceQuotaMap = Map<number, number>;

/** Build tier → count for a desired [min, max] price range (e.g. tops 100–200). */
export const buildPriceQuotas = (priceMin: number, priceMax: number): PriceQuotaMap => {
  const quotas: PriceQuotaMap = new Map();

  const inRange = (tier: number): boolean => tier >= priceMin && tier <= priceMax;
...
export const totalQuotaCount = (quotas: PriceQuotaMap): number => {
  let sum = 0;
  for (const n of quotas.values()) sum += n;
  return sum;
};

export const quotasToRecord = (quotas: PriceQuotaMap): Record<string, number> => {
...
export const scaleQuotasToLimit = (
  quotas: PriceQuotaMap,
  limit: number
): PriceQuotaMap => {
  const total = totalQuotaCount(quotas);
  if (!Number.isFinite(limit) || limit <= 0) {
    throw new Error('--limit must be a positive number');
  }
  if (limit >= total) return new Map(quotas);

  const tiers = [...quotas.keys()].sort((a, b) => a - b);
  const scaled: PriceQuotaMap = new Map();
  let assigned = 0;
...
export const randomPriceInRange = (min: number, max: number): number => {
